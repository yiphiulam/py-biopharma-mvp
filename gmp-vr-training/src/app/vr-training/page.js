...
// 動態載入 VRAliquotingScene，停用 SSR
const VRAliquotingScene = dynamic(() => import('../../components/VRAliquotingScene'), { 
  ssr: false,
  loading: () => (
...
export default function VRTraining() {
  const { startSimulation, currentStepIndex } = useTrainingStore();
  ...
  // Hover-to-Advance: mouse / gaze enter triggers click after 1.5 s
  useEffect(() => {
    let cleanupHandlers = [];
    const timeoutId = setTimeout(() => {
      ...
      clickables.forEach(el => {
        ...
        const onEnter = () => {
          timer = setTimeout(() => {
            el.emit('click'); // triggers A‑Frame click after 1.5 s
          }, 1500);
        };
        const onLeave = () => {
